...
def select_file():
    """
    Opens windows allowing the user to find a file to open. *.txt are default.
    """
    # Defining filetypes
    filetypes = (
        ('Text files', '*.txt'),
    ...
    # Creating window from tkinter lib
    filename = filedialog.askopenfilename(
        title='Open text file',
        initialdir='/',
        filetypes=filetypes)
    # Read the file saving the contents as `contents`
    with open(filename, 'r') as f:
        contents = f.read()
    # Clearing input_box and inserting `contents`
    text.delete('1.0', END)
    text.insert('end', contents)


def save_file():
    """Saves file for user to open later. Format is *.txt"""
    # Opening the window from the tkinter lib
    ...
    # Opening a file (specified as `filename`) and writing `final_file` to disk
    final_file = open(filename, "w+")
    final_file.write(text.get(1.0, 'end-1c'))


def check_for_google_protocol():
    # Checks if the protocol is Google and updates the `rate` input box,
    while True:
...
def get_help():
    ...
    # Creating new window and defining size, icon, and title
...
# win1.rowconfigure(3, weight=10)
# win1.rowconfigure(4, weight=10)
...
# Creating a button that opens a dialog for the user to select a file to be autofilled
